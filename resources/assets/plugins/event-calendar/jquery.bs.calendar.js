(function ($) {
	...
	nextYear= new Date(year + 1, month, day);
	...
	function drawNoEvent(date) {
        ...
        <div class="p-2" style="font-size:.8em">
				<div class="d-flex">
						<div class="w-50 form-group pr-2">
								<label>From Date</label>
								<input type="text" class="form-control" placeholder="Date">
						</div>
					<div class="w-50 pl-2 form-group">
							<label>To Date</label>
							<input type="text" class="form-control" placeholder="Date">
					</div>
				</div>
	...
	function drawTemplate(container) {
		...
		container
			...
			.addClass('bg-white')
			.html(`
			<div class="p-3 d-flex flex-column text-muted bs-calendar-header">
				<small>${today.getDayName()}</small>
				<h4 class="mb-0">${today.getDate()}. ${today.getMonthName()} ${today.getFullYear()}</h4>
			</div>
            <div class="d-flex flex-nowrap justify-content-between align-items-center mb-2 pb-2 border-bottom">
                <a href="#" class="btn btn-light btn-prev-month bg-transparent shadow-none border-0"><i class="${settings.icons.prev}"></i></a>
                <a href="#" class="btn btn-light mx-1 flex-fill btn-curr-month month-name bg-transparent shadow-none border-0"></a>
                <a href="#" class="btn btn-light btn-next-month bg-transparent shadow-none border-0"><i class="${settings.icons.next}"></i></a>
            </div>
            <div class="d-flex flex-nowrap align-items-center js-weekdays justify-content-center">
                <div class="text-center"></div>
            </div>
             <div class="js-weeks"></div>
            <div class="dates"></div>
            <div class="pt-2 js-collapse" style="display: none">
                <div class="card mb-0 border-top border-bottom shadow-none rounded-0">
                    <div class="fw-bold p-2 js-day-name d-flex justify-content-between align-items-center"><span></span>
                    <a href="javascript:;" class="p-2 text-primary add-specific-ws-hours"> <i class="fa fa-plus"></i> </a>
                    </div>
                    <div class="new-specific-time-slots"></div>
	...
	$.fn.bsCalendar = function (options, params) {
		...
		let cellWidthHeight = 330 / 8; // 7 days + calendar week
		...
		function drawCalendar(selectedDate, clickDate = null) {
			...
			var specificArray = [];
			var locationId = $('#calendar_offcanvas').attr('location-id');
            var wsId = $('#calendar_offcanvas').attr('ws-id');
            if(_hoursby == 2) { // Location Specific Array
            	if (locationId != undefined && wsId != undefined) {
            		let dtLocationSpecific = $.parseJSON(dtLocationSpecificArray);
            		var specificArray = ((locationId in dtLocationSpecific) && (wsId in dtLocationSpecific[locationId])) ? dtLocationSpecific[locationId][wsId] : [];	
            	}
            } else { // Company Specific Array
            	if (wsId != undefined && wsId != undefined && dtSpecificArray != undefined) {
            		let dtSpecific = $.parseJSON(dtSpecificArray);
                	var specificArray = (wsId in dtSpecific) ? dtSpecific[wsId] : [];
                }
            }
			...
			calendar.forEach(week => {
				...
				week.days.forEach(day => {
					let isBold = (jQuery.inArray(day.formatDate(false), specificArray) !== -1);
					let isToday = today.formatDate(false) === day.formatDate(false);
					let inMonth = selectedDate.formatDate(true)[1] === day.formatDate(true)[1];
					let cellBackground = isToday ? 'text-primary' : (inMonth ? ' bg-white ' : ' fs-6 fw-small');
					let cellTextColor = inMonth ? 'var(--bs-dark)' : '#adadad';
					let highlight = !isToday ? '' : ' js-today ';
					let boldClass = isBold ? ' fw-bold' : '';

					var disableDate = true;
					var colJson = {
						'data-date': day.formatDate(false),
						class: 'position-relative d-flex border border-white rounded-circle justify-content-center align-items-center' + highlight + cellBackground + boldClass,
						css: {
							color: cellTextColor,
							cursor: 'pointer',
							width: cellWidthHeight,
						...
						html: [
							...
							[
								'<small class="js-count-events position-absolute text-center rounded-circle bg-primary" style="width:4px; height:4px; bottom:4px; left: 50%; margin-left:-2px">',

								'</small>'
							].join('')
						].join('')
					};
					if(Date.parse(selectedDate) > Date.parse(nextYear)) {
						disableDate = false;
						var colJson = {
							class: 'position-relative d-flex border border-white text-secondary rounded-circle justify-content-center align-items-center' + highlight + boldClass,
							css: {
								color: cellTextColor,
								cursor: 'pointer',
								width: cellWidthHeight,
					...
					let col = $('<div>', colJson).appendTo(weekContainer);
					col.data('events', []);
				});
			});
			...
			getEvents(container, selectedDate, function (events) {
				...
				// } else
					...
					if (container.find('.js-today').length) {
						...
						container.find('.js-day-name span').html(selectedDate.showDateFormatted());
		...
		function events() {
			container
				...
				.on('click', '[data-date]', function (e) {
					...
					container
						.find('.js-weeks')
						.find('[data-date].border-primary')
						.removeClass('border-primary active')
						.addClass('border-white')
					$column.removeClass('border-white').addClass('border-primary active')
					...
					if(Date.parse(moment(SPECIFICDATE).format("YYYY-MM-DD")) < Date.parse(moment(Date()).format("YYYY-MM-DD"))) {
						container.find('.js-collapse').find('.add-specific-ws-hours').hide();
					} else {
						container.find('.js-collapse').find('.add-specific-ws-hours').show();
					}
					let events = $column.data('events');
					container.find('.js-day-name span').html(date.showDateFormatted());
