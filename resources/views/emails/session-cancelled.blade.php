                        ...
                        <tr align="center" style="border-collapse:collapse;text-align: center">
                            <td style="padding-top:70px;padding-left: 15px;padding-right: 15px;">
                                <p style="margin: 0;line-height: 1.4;font-size: 18px;color: #333333;">
                                    Hi @if($cancelledBy == 'user' || $cancelledBy == 'zca')
                                    {{$wsFirstName}},
                                    @else
                                    {{$userFirstName}}, 
                                    @endif<br/><br/>
                                    ...
                                    @if($cancelledBy == 'user')
                                        Your {{$serviceName}} session with {{$userName}} on {{$eventDate}} has been cancelled.
                                    @elseif($cancelledBy == 'zca')
                                        Your {{$serviceName}} session has been cancelled.
                                    @else
                                        Your {{$serviceName}} session with {{$wsName}} on {{$eventDate}} has been cancelled.
                                    @endif
                                    ...
                                    <table style="text-align: left;font-size: 18px;margin: auto;">
                                        ...
                                        @endif
                                        @if($cancelledReason && $cancelledBy != 'zca')
                                        <tr>
                                            <td style="font-weight: bold; width:40%;">
                                                Reason:
                                            </td>
