...
<a class="action-icon" href="{{ route('admin.companiesold.edit', [$roleGroupCompany ,$record->id]) }}" title="{{ trans('labels.buttons.edit_record') }}">
    <i class="far fa-edit">
    </i>
</a>
@endauth
@permission('add-moderator')
<a class="action-icon" href="{{ route('admin.companiesold.createModerator', [$roleGroupCompany, $record->id, 'referrer' => 'index']) }}" title="Add Moderator">
    <i class="far fa-plus">
    </i>
</a>
@endauth
...
<div class="dropdown d-inline-block">
    ...
    <div aria-labelledby="dropdownMenuButton" class="dropdown-menu dropdown-menu-right">
        ...
        <a class="action-icon dropdown-item" href="{{ route('admin.companiesold.teams', [$roleGroupCompany, $record->id]) }}" title="Get Teams">
            <i class="far fa-users me-2">
            </i>
            Get Teams
        </a>
        @endauth
        ...
        <a class="action-icon dropdown-item" href="{{ route('admin.companiesold.changeAppSettingIndex', [$roleGroupCompany, $record->id]) }}" title="App Setting">
            <i class="far fa-cog nav-icon me-2">
            </i>
            App Setting
        </a>
        @endif
        ...
        <a class="action-icon dropdown-item" href="{{ route('admin.companiesold.moderators', [$roleGroupCompany, $record->id]) }}" title="View Moderator">
        ...
        @if($role->group == 'zevo')
        <a class="action-icon dropdown-item" href="{{ route('admin.companiesold.getLimits', [$roleGroupCompany, $record->id]) }}" title="View Limits">
            <i class="far fa-microchip me-2">
            </i>
            View Limits
        </a>
        @endif
        ...
        <a class="action-icon dropdown-item masterclass-survey-report" data-id="{{ $record->id }}" href="javascript:void(0);" title="Export Masterclass Survey Report">
            <i class="far fa-file-chart-line me-2">
            </i>
        ...
        @if($record->is_reseller && is_null($record->parent_id))
        <a class="action-icon dropdown-item" href="{{ route('admin.companiesold.portalFooter', [$roleGroupCompany, $record->id]) }}" title="Portal Footer">
            <i class="far fa-info-circle me-2">
            </i>
            Portal Footer
        </a>
        @endif
