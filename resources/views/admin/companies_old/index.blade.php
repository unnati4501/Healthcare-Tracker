...
@include('admin.companies_old.breadcrumb', [
    ...
    'breadcrumb' => Breadcrumbs::render('companiesold.index'),
    ...
    <div class="container-fluid">
        ...
        <div class="card search-card">
            <div class="card-body pb-0">
                ...
                {{ Form::open(['route' =>  ['admin.companiesold.index', $companyType], 'class' => 'form-horizontal', 'method' => 'get', 'role' => 'form', 'id' => 'companySearch']) }}
                ...
                <div class="search-outer d-md-flex justify-content-between">
                    ...
                    <div class="search-actions align-self-start">
                        ...
                        <a class="filter-cancel-icon" href="{{ route('admin.companiesold.index',$companyType) }}">
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        $('#companyManagment').DataTable({
            ...
            serverSide: true,
            ajax: {
                url: '{{ route('admin.companiesold.getCompanies','zevo') }}',
                data: {
                    status: 1,
        ...
        $(document).on('click', '#delete-model-box-confirm', function (e) {
            ...
            $.ajax({
                type: 'DELETE',
                url: "{{route('admin.companiesold.delete','/')}}"+ '/' + objectId,
                data: null,
                crossDomain: true,
                cache: false,
                contentType: 'json',
        ...
        $(document).on('click', '.export-survey-report', function(e) {
            var id = ($(this).data('id') || 0),
                url = "{{ route('admin.companiesold.get-survey-details', [':company_id', 'zcsurvey']) }}",
                frmUrl = "{{ route('admin.companiesold.export-survey-report', [':company_id', 'zcsurvey']) }}",
        ...
        $(document).on('click', '.masterclass-survey-report', function(e) {
            var id = ($(this).data('id') || 0),
                url = "{{ route('admin.companiesold.get-survey-details', [':company_id', 'masterclass']) }}",
                frmUrl = "{{ route('admin.companiesold.export-survey-report', [':company_id', 'masterclass']) }}",
                frmUrl = frmUrl.replace(':company_id', id);
            $.ajax({
                url: url.replace(':company_id', id),
                type: 'GET',
