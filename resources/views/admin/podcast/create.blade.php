...
@include('admin.podcast.breadcrumb', [
    'mainTitle' => trans('podcast.title.add'),
    'breadcrumb' => Breadcrumbs::render('podcasts.create'),
])
<!-- /.content-header -->
@endsection
...
<section class="content no-default-select2">
    <div class="container-fluid">
        <div class="card form-card">
            {{ Form::open(['route' => 'admin.podcasts.store', 'class' => 'form-horizontal zevo_form_submit', 'method' => 'post', 'role' => 'form', 'id' => 'podcastAdd', 'files' => true]) }}
            <div class="card-body">
                @include('admin.podcast.form', ['edit' => false])
            </div>
...
{!! JsValidator::formRequest('App\Http\Requests\Admin\CreatePodcastRequest','#podcastAdd') !!}
...
<script type="text/javascript">
    ...
    messages.choose_label = `{{ trans('podcast.form.placeholder.choose_label') }}`;
    var upload_image_dimension = '{{ trans('podcast.messages.upload_image_dimension') }}';
    function formSubmit() {
        $('#podcastAdd').valid();
        var podcastCompany = $('#podcast_company').val().length;
        if (podcastCompany == 0) {
...
<script src="{{ mix('js/podcast/create.js') }}" type="text/javascript">
</script>
@endsection
