<div class="col-md-6">
    <div class="d-flex">
        ...
        <div class="col-md-10">
            ...
            <span class="gray-500">
                {{$note['notesAddedBy']}} | {{ Carbon\Carbon::parse($note['created_at'])->format(config('zevolifesettings.date_format.date_format_for_client_notes'))}}
            </span>
        </div>
        {{-- <div class="editnote" data-id="{{$note['id']}}"style="float: right;margin-left: 200px;"><i class="far fa-edit"></i>&nbsp;&nbsp;&nbsp;<i class="far fa-trash-alt"></i></div> --}}
         <div class="editdelenotes col-md-2">
            @php
                $scheduleId = (!empty($scheduleId) ? $scheduleId : 0);
            @endphp
            @if(auth()->user()->id == $note['user_id'])
            <a href="javascript:;" class="open-editNotes-model" data-bs-toggle="modal" data-noteFromTable="{{$note['fromTable']}}" data-bs-target="#edit-note-model" data-id="{{$note['id']}}" data-clientid="{{$scheduleId}}" > <i class="far fa-edit"></i></a>
            <a class="action-icon danger" data-id="{{$note['id']}}" data-notefrom="ClientNote" data-noteFromTable="{{$note['fromTable']}}" href="javaScript:void(0)" id="clientNoteDelete" title="{{ trans('challenges.buttons.tooltips.delete') }}">
