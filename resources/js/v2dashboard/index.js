 ...
 */
var tabs = ['usage', 'behaviour', 'psychological', 'booking'];
if ($('ul.dashboard-tabs #audit').length > 0) {
    tabs.push('audit');
}
...
$(document).ready(function() {
    ...
    $('#dashboardTabs').easyResponsiveTabs({
        ...
        tabidentify: 'dashboard-tabs', // The tab groups identifier
...
function loadTabData(options) {
    ...
    $('#company').removeClass('col-md-6 ms-auto').addClass('col');
    $('#department,#age').show();
    switch (currActiveTab) {
        ...
        case 'booking':
            $('#department,#age').hide();
            $('#company').removeClass('col').addClass('col-md-6 ms-auto');
            // Upcoming Events
...
$('#company_id').change(function() {
    ...
    if ($('#company_id').val() != '' && $('#company_id').val() != null) {
        ...
        url = urls.deptUrl.replace(':id', value);
        $.ajax({
            url: url,
            method: 'get',
            data: {
    ...
    $.ajax({
        ...
        success: function(result) {
            if (result.result.flag == false) {
                $('#meditationhours').hide();
                $('#behaviour').hide();
                var checkActiveBehaviourTab = $('#behaviour.resp-tab-active').length;
                if (checkActiveBehaviourTab >= 1) {
                    $('#usage').click();
                }
            } else {
                $('#meditationhours').show();
                $('#behaviour').show();
            }
        }
    });
