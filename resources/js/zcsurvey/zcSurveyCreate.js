...
$(document).ready(function() {
    ...
    datatables.allquestions = $('#allquestions').DataTable({
        ...
        rowCallback: function(row, data, displayNum, displayIndex, dataIndex) {
            ...
            if (datatables.surveyquestions.rows(`[data-id="${data.id}"]`).any()) {
    ...
    datatables.finalQuestionList = $('#finalQuestionList').DataTable({
        ...
        columns: [{
            ...
            render: function(data, type, row) {
                ...
                <a class="action-icon danger delete_question" href="javascript:void(0);" title="Delete" data-id="${row.id}">
    ...
    $(document).on('click', '#addToSurveyQuestions', function(e) {
        ...
        if ($rows.length > 0) {
            ...
            // datatables.surveyquestions.rows.add(addRows.reverse()).draw();
