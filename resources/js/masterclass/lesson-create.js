...
$(document).ready(function() {
    ...
    CKEDITOR.ClassicEditor.create(document.querySelector( '.article-ckeditor' ), {
        ...
        contentCkeditor.model.document.on( 'change:data', () => {
                if ($('#description-error-cstm').length) {
                    var description = contentCkeditor.getData();
                    console.log("description: "+ description);
                    description = $(description).text().trim();
                    if(description == ''){
                        $('#description-error-cstm').html("The description field is required").addClass('invalid-feedback').show();
                    } else {
                        $('#description-error-cstm').html("").removeClass('invalid-feedback').hide();
                    }
                }
             } );
    } )
    ...
    $('#video_duration').on('loadedmetadata', function(e) {
        ...
        if (time > 59) {
            ...
            $("#courseLessionAdd").validate().element("#duration");
        } else {
            toastr.error(messages.video_length_limit);
            $('#video').val('');
            $('#video').parent('div').find('.custom-file-label').html(messages.choosefile);
            $('#duration').val('');
        }
    ...
    $("#audio_duration").on("canplaythrough", function(e) {
        ...
        if (time > 59) {
            ...
            $('#audio').val('');
            $('#audio').parent('div').find('.custom-file-label').html(messages.choosefile);
            $('#duration').val('');
        }
        URL.revokeObjectURL(objectUrl);
    });
    $(document).on('change', '#lesson_type', function(e) {
        ...
        $('#main_wrapper').find('.custom-file-label').html(messages.choosefile);
        $('#main_wrapper').find('img').prop('src', bg_background);
        $('#main_wrapper').find('input, textarea').removeClass('is-invalid is-valid');
        contentCkeditor.setData('');
    ...
    $('#courseLessionAdd').ajaxForm({
        beforeSend: function() {
            $('.progress-loader-wrapper .status-text').html(messages.uploading_media);
            $('.progress-loader-wrapper').show();
            $('#courseLessionAdd .card-footer button, #courseLessionAdd .card-footer a').attr('disabled', 'disabled');
            var percentVal = '0%';
        ...
        success: function(data) {
            ...
            $('#courseLessionAdd .card-footer button, #courseLessionAdd .card-footer a').removeAttr('disabled');
            var percentVal = '100%';
            bar.width(percentVal)
            percent.html(percentVal);
...
$(document).on('click', '#zevo_submit_btn', function() {
    ...
    if (description == '') {
        ...
        $('#description-error-cstm').html("The description field is required").addClass('is-invalid').show();
        isError = 0;
    } else {
