$(document).ready(function() {
    ...
    $(document).on('click', '#addSubcategory', function(t) {
        var serviceConfirmModalBox = '#subcategory-model-box';
        $('#sub_category_logo-error,#sub_cateory_name-error').remove();
        $("#modal_title").html(labels.add_subcategory);
        $(serviceConfirmModalBox).attr("data-id", $(this).data('id'));
        $(serviceConfirmModalBox).modal('show');
        //$("#addSubcategoryForm").attr('action','');
    ...
    // Validator method to add only letter and space
    $.validator.addMethod("lettersAndSpaceOnly", function(value, element) {
        return this.optional(element) || /^[a-z \s]+$/i.test(value);
        }, messages.enter_valid_subcategory);
    
    $("#subcategory_remove_1").hide();
    ...
    $(document).on('click', '#subcategorySave', function(e) {
        e.preventDefault();
        $('#sub_category_logo-error, #sub_cateory_name-error').remove();
        var res = [];
        ...
        if(res.length <= 0){
            ...
            var subCategoryLogo = $('#sub_category_logo_preview').attr('src');
    ...
    $(document).on('click', '.subcategory-remove', function(t) {
        ...
        $(deleteConfirmModalBox).attr("data-id", $(this).closest("tr").attr('data-order'));
    ...
    $('#delete-model-box-confirm').on('click', function(e) {
        ...
        if (id) {
            $("tr").filter("[data-order='" + id + "']").remove();
    ...
    $(document).on('click', '#subcategory-model-box #subcategoryUpdate', function(t) {
        $('#sub_category_logo-error, #sub_cateory_name-error').remove();
        var res = [];
        if($("label[for='sub_category_logo'].custom-file-label").html() == 'Choose File'){
            $('#sub_category_logo').after('<div id="sub_category_logo-error" class="error text-danger">'+messages.field_required+'</div>');
