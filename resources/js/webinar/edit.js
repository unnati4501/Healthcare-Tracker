...
$(document).ready(function() {
    $(document).on('click','#webinarSubmit',function(){
        $('#webinaredit').valid();
        var webinarCompany = $('#webinar_companys').val().length;
        if (webinarCompany == 0) {
            event.preventDefault();
            $('#webinaredit').valid();
            $('#webinar_companys').addClass('is-invalid');
            $('#webinar_companys-error').show();
            $('.tree-multiselect').css('border-color', '#f44436');
        } else {
            $('#webinar_companys').removeClass('is-invalid');
            $('#webinar_companys-error').hide();
    ...
    $(document).on('change', '#webinar_type', function(e) {
        var _value = $(this).val(),
    ...
    $(document).on('change', '#webinar_cover, #header_image', function (e) {
    ...
    $('#video_duration').on('loadedmetadata', function(e) {
        ...
        if(time > 59) {
            ...
            $("#webinaradd").validate().element("#duration");
        } else {
            toastr.error(message.video_1minute_log);
            $('#video').val('');
            $('#video').parent('div').find('.custom-file-label').html('Choose File');
        }

        URL.revokeObjectURL(objectUrl);
    });

    $('#webinaredit').ajaxForm({
        beforeSend: function() {
            $('.progress-loader-wrapper .status-text').html(message.uploading_media);
            $('.progress-loader-wrapper').show();
            $('#webinaredit .card-footer button, #webinaredit .card-footer a').attr('disabled', 'disabled');
            var percentVal = '0%';
        ...
        success: function(data) {
            ...
            $('#webinaredit .card-footer button, #webinaredit .card-footer a').removeAttr('disabled');
            var percentVal = '100%';
            bar.width(percentVal)
            percent.html(percentVal);
            if(data.status && data.status == 1) {
                window.location.replace(url.webinarIndex);
            } else {
    ...
    $("#webinar_companys").treeMultiselect({
    ...
    var _trackType = $('#webinar_type').val(),
    duration_readonly = true;
    if(_trackType != "") {
        $('#youtube_wrapper, #video_wrapper').hide();
        if(_trackType == '1') {
            $('#video_wrapper').show();
