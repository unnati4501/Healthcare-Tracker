...
class SendSessionConsentEmail extends Mailable implements ShouldQueue
{
    ...
    public function build()
    {
        try {
            ...
            $subject             = "1:1 Session Consent Form";
            $portaldomain        = null;
            ...
            $dataArray = [
                ...
                'consentUrl'          => (!empty($this->data['consentUrl']) ? $this->data['consentUrl'] : ""),
                'logo'                => $logo,
            ...
            $mail = $this
                ->from($address, $name)
                ->subject($subject)
                ->view('emails.session-consent', $dataArray);

            // Update in cronofy schedule is mail send to user
            CronofySchedule::where('id', $this->data['sessionId'])->update(['is_consent' => true]);
