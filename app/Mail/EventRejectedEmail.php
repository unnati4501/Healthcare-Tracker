...
class EventRejectedEmail extends Mailable implements ShouldQueue
{
    ...
    public function build()
    {
        ...
        $subject           = "{$eventName} - Event Rejected";
        $portaldomain      = null;
        $isReseller        = false;
        $title             = $eventName . " - Event Rejected" ;
        ...
        $mail = $this
            ->from($address, $name)
            ->subject($subject)
            ->view('emails.eventrejected', $dataArray);

        if (!empty($this->data['iCal'])) {
            $mail->attachData($this->data['iCal'], 'event-rejected.ics', [
