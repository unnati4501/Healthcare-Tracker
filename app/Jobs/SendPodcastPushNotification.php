...
class SendPodcastPushNotification implements ShouldQueue
{
     ...
     * @return void
     */
    public function __construct(Podcast $podcast, $string, $newlyAssociatedComps = array())
    {
    ...
    public function handle()
    {
        if ($this->newlyAssociatedComps) {
            $membersData = User::select('users.id', 'users.first_name', 'user_notification_settings.flag AS notification_flag')
                ...
                ->leftJoin('user_notification_settings', function ($join) {
                    $join->on('user_notification_settings.user_id', '=', 'users.id')
                        ...
                        ->whereRaw('(`user_notification_settings`.`module` = ? OR `user_notification_settings`.`module` = ?)', ['podcasts', 'all']);
                })
                ->whereRaw("user_team.team_id IN (?)",[implode(',', $this->newlyAssociatedComps)])
            ...
            $membersData = User::select('users.id', 'users.first_name', 'user_notification_settings.flag AS notification_flag')
                ...
                ->whereRaw(DB::raw('user_team.team_id IN ( SELECT team_id FROM `podcast_team` WHERE podcast_id = ? )'),[
                    $this->podcast->id
                ])
                ->where(['is_blocked' => false])
                ->groupBy('users.id')
        ...
        $title = trans('notifications.podcast.podcast-added.title');
        $planAccess   = true;
        if ($membersData->count() > 0) {
            foreach ($membersData as $value) {
                ...
                if($planAccess){
                    $message = trans('notifications.podcast.podcast-added.message', [
                        'first_name'  => $value->first_name,
                        'track_title' => $this->podcast->title,
                    ]);
    
                    $notificationData = [
                        'type'          => 'Auto',
                        'creator_id'    => $this->podcast->coach_id,
                        'title'         => $title,
                        'message'       => $message,
                        'push'          => true,
                        'scheduled_at'  => now()->toDateTimeString(),
                        'deep_link_uri' => $this->podcast->deep_link_uri,
                        'is_mobile'     => config('notification.podcast.added.is_mobile'),
                        'is_portal'     => config('notification.podcast.added.is_portal'),
                        'tag'           => 'podcast',
                    ];
