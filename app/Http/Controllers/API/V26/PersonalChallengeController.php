...
use App\Http\Collections\V26\PersonalChallengeListCollection;
use App\Http\Controllers\API\V25\PersonalChallengeController as v25PersonalChallengeController;
...
use App\Http\Resources\V26\PersonalChallengeDetailResource;
use App\Http\Resources\V26\PersonalChallengeHistoryDetailResource;
...
class PersonalChallengeController extends v25PersonalChallengeController
{
    ...
    public function joinPersonalChallenge(JoinPersonalChallengeRequest $request, PersonalChallenge $personalChallenge)
    {
        try {
            ...
            if ($personalChallenge->challenge_type == 'habit') {
                $endDate = $payload['endDate'] . ' 23:59:59';
                $endDate = Carbon::parse($endDate, $user->timezone)->setTimezone(config('app.timezone'))->toDateTimeString();
            } else {
    ...
    public function store(CreatePersonalChallengeRequest $request)
    {
        try {
            ...
            $personalChallengeUserTaskInput = [];
            if ($record->type == 'streak' || $payload['challengetype'] == 'habit') {
    ...
    public function update(UpdatePersonalChallengeRequest $request, PersonalChallenge $personalChallenge)
    {
        try {
            ...
            if ($record) {
                ...
                if ($payload['type'] == 'streak' || $payload['challengetype'] == 'habit') {
                    $taskData = $personalChallenge->personalChallengeTasks()->first();
