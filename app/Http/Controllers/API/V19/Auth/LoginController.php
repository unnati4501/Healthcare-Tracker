...
namespace App\Http\Controllers\Api\V19\Auth;

use App\Http\Collections\V17\AppSettingCollection;
use App\Http\Controllers\API\V18\Auth\LoginController as v18LoginController;
use App\Http\Controllers\Controller;
use App\Http\Requests\Api\V1\EmailRequest;
use App\Http\Requests\Api\V1\LoginRequest;
use App\Http\Resources\V18\UserResource;
 ...
 */
class LoginController extends v18LoginController
{
            ...
            if (!empty($user)) {
                ...
                foreach ($defaultLabelString as $groupKey => $groups) {
                    foreach ($groups as $labelKey => $labelValue) {
                        $companyLabelString[$labelKey] = ($companyLabelString[$labelKey] ?? $labelValue['default_value']);
                    }
                }
            } else {
                ...
                foreach ($defaultLabelString as $groupKey => $groups) {
                    foreach ($groups as $labelKey => $labelValue) {
                        $companyLabelString[$labelKey] = $labelValue['default_value'];
                    }
                }
            }
