...
namespace App\Http\Controllers\Api\V30\Auth;

use App\Http\Controllers\API\V29\Auth\RegisterController as v29RegisterController;
 ...
 */
class RegisterController extends v29RegisterController
{
            ...
            if (!empty($user)) {
                if ($xDeviceOs != config('zevolifesettings.PORTAL') && $user->can_access_app) {
                    ...
                    if (empty($request->get('socialId'))) {
                        if ($xDeviceOs == config('zevolifesettings.PORTAL') && !empty($request->get('password'))) {
