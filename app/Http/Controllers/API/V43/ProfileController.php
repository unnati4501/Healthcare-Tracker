...
use App\Http\Controllers\API\V34\ProfileController as v34ProfileController;
use Illuminate\Auth\AuthenticationException;
use Illuminate\Http\JsonResponse;
use Illuminate\Http\Request;
use App\Http\Requests\Api\V1\EditProfileRequest;
use App\Http\Resources\V43\UserProfileResource;
use App\Http\Traits\ProvidesAuthGuardTrait;
use App\Http\Traits\ServesApiTrait;
use Carbon\Carbon;

class ProfileController extends v34ProfileController
{
        ...
        try {
            ...
            if ($existingProfile->age != $age) {
                ...
                if (!empty($lastWeight) && $existingProfile->height) {
                    ...
                    $user->bmis()
                        ->whereRaw("DATE(CONVERT_TZ(user_bmi.log_date, ?, ?)) = ?",[
                            $appTimezone,$user->timezone,$weightCurrentDateTimeZone->toDateString()
                        ])
                        ->get()->each->delete();
