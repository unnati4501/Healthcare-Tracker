...
use App\Http\Controllers\API\V40\EventController as v40EventController;
...
class EventController extends v40EventController
{
    /**
    ...
    public function index(Request $request, $type = 'registered')
    {
        try {
            ...
            ->selectRaw("CONVERT_TZ(CONCAT(event_booking_logs.booking_date, ' ', event_booking_logs.start_time), ?, ?) AS event_date_time",[
            ...
            if ($type == 'booked') {
                ...
                $eventRecords
                    ...
                    ->whereRaw("IF((? BETWEEN CONVERT_TZ(CONCAT(event_booking_logs.booking_date, ' ', event_booking_logs.start_time), ?, ?) AND ADDTIME(CONVERT_TZ(CONCAT(event_booking_logs.booking_date, ' ', event_booking_logs.start_time), ?, ?), events.duration) AND (event_registered_users_logs.is_cancelled IS NULL OR event_registered_users_logs.is_cancelled = 1)), 0, 1)",[
                        $now,$appTimezone,$timezone,$appTimezone,$timezone
                    ])
            ...
            $sessionDetails =  CronofySchedule::leftJoin('session_group_users', 'session_group_users.session_id', '=', 'cronofy_schedule.id')
            ->where(function ($q) use ($user) {
                $q->where('session_group_users.user_id', $user->id);
                $q->orWhere('cronofy_schedule.user_id', $user->id);
            })
            ->select(
                'cronofy_schedule.id',
            ...
            if (!empty($request->month)) {
                ...
                $sessionDetails
                    ->whereRaw("TIMESTAMP(cronofy_schedule.start_time) BETWEEN ? AND ?",[
                        $fromdate,$todate
                    ]);
            }
