...
namespace App\Http\Controllers\Api\V43\Auth;

use App\Http\Collections\V43\AppSettingCollection;
use App\Http\Controllers\API\V42\Auth\LoginController as v42LoginController;
use App\Http\Controllers\Controller;
use App\Http\Requests\Api\V1\EmailRequest;
use App\Http\Requests\Api\V1\LoginRequest;
use App\Http\Resources\V43\UserAutoFillResource;
use App\Http\Resources\V33\UserResource;
 ...
 */
class LoginController extends v42LoginController
{
    use HasApiTokens, AuthenticatesUsers, ServesApiTrait, ProvidesAuthGuardTrait {
    ...
    public function attemptLogin(Request $request)
    {
        ...
        if (!empty($request->get('email')) && !empty($request->get('socialId'))) {
            ...
            $token = $userData->createToken('email')->accessToken;
            ...
            $getUserDetails = User::where('email', $request->get('email'))->first();
            $user           = User::join("user_otp", "user_otp.email", "=", "users.email")
                ...
                ->first();
            if ($user) {
                return $getUserDetails->createToken('email')->accessToken;
            } else {
                return false;
            }
        }
    ...
    public function sendLockoutResponse(Request $request): void
    {
        throw ValidationException::withMessages([
            $this->username() => \trans('api_labels.auth.throttle'),
        ])->status(422);
    }
    ...
    public function authenticated(Request $request, $user, $token): JsonResponse
    {
        ...
        if ($companyDetails) {
            if ($xDeviceOs == config('zevolifesettings.PORTAL')) {
                ...
                $userData['dob']         = Carbon::parse($user->profile->birth_date)->toDateString();
                $userData['location']    = [
                    'id'   => $location->id,
                    'name' => $location->name,
                ];
            ...
            if ($xDeviceOs == config('zevolifesettings.PORTAL') && $role->slug != 'user' && !$user->can_access_portal) {
                Token::where('user_id', $user->id)->orderBy('created_at', 'asc')->first()->revoke($token);
    ...
    public function sendSingleUseCode(EmailRequest $request, $fromApi = 'login')
    {
        try {
            ...
            $userOtpExists    = UserOtp::where('email', $request->input('email'))->get();
            ...
            if($userOtpExists->count() > 3){
                UserOtp::where('email', $request->input('email'))->orderBy('id', 'ASC')->limit(1)->delete();
            }
            ...
            $findPreviousRecords = UserOtp::where('id', '!=', $record->id)->where('email', $request->input('email'))
                ->update([
    ...
    public function verifySingleUseCode(Request $request)
    {
        try {
            ...
            $getLast3Records    = UserOtp::where('email', $request->get('email'))->orderBy('id', 'DESC')->limit(3)->offset(1)->get()->toArray();
            ...
            if (empty($userData)) {
                ...
                throw ValidationException::withMessages([
                    'Invalid OTP' => trans('api_labels.auth.invalid_otp'),
                ])->status(422);
            } else if (is_null($codeExpired)) {
                throw ValidationException::withMessages([
                    'OTP Expired' => trans('api_labels.auth.otp_expired'),
                ])->status(422);
            } else {
