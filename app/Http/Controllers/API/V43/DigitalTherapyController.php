...
use App\Http\Collections\V37\DigitalTherapyCollection;
use App\Http\Collections\V43\DigitalCounsellorListCollection;
use App\Http\Controllers\API\V39\DigitalTherapyController as v39DigitalTherapyController;
...
use App\Http\Resources\V38\CronofySessionDetailsResource;
 ...
 */
class DigitalTherapyController extends v39DigitalTherapyController
{
    ...
    public function index(Request $request)
    {
        try {
            ...
            $user           = $this->user();
            $company        = $user->company()->select('companies.id', 'companies.is_reseller', 'companies.code', 'companies.eap_tab', 'companies.parent_id')->first();
            ...
            $slideRecords = CompanyDigitalTherapyBanner::where('company_id', $company->id)->orderBy("order_priority", "ASC")->get();
            ...
            // Display the static banners as per company type
            if ($slideRecords->count() == 0) {
                if (!empty($company) && !$company->is_reseller && is_null($company->parent_id)) {
                    // zevo company sliders
                    $sliders = json_decode(json_encode($zevoBanners), false);
                } else {
                    // parent/child company sliders
                    $sliders = json_decode(json_encode($parentChildBanners), false);
                }
                $staticSliderRecords = collect($sliders);
            }
            ...
            $data = [
                ...
                'sliders'                => ($slideRecords->count() > 0 ? $slideRecords : $staticSliderRecords),
    ...
    public function counsellorListing(Request $request, ServiceSubCategory $serviceSubCategory)
    {
        try {
            ...
            $cronofySchedule = $loginUser->bookedCronofySessions()
                ...
                ->whereNotIn('status', ['canceled', 'short_canceled', 'open', 'completed', 'rescheduled'])
                ->select('ws_id')
                ->distinct()
                ->get()
                ->pluck('ws_id')
                ->toArray();
            $getWellbeingSpecialist = User::select(\DB::raw("CONCAT(users.first_name, ' ', users.last_name) AS name"), 'users.id', 'user_profile.about', 'user_profile.gender', 'ws_user.language')
            ...
            if ($setHoursBy == 1 && $setAvailabilityBy == 1) {
                ...
                $getWellbeingSpecialist->join('digital_therapy_specific', 'digital_therapy_specific.ws_id', '=', 'users.id')
                    ...
                    ->where(\DB::raw("CONCAT(DATE_FORMAT(date, '%Y-%m-%d'), ' ', end_time)"), '>=', Carbon::now()->toDateTimeString());
            } elseif ($setHoursBy == 1 && $setAvailabilityBy == 2) {
                $getWellbeingSpecialist->join('digital_therapy_specific', 'digital_therapy_specific.ws_id', '=', 'users.id')
                    ->whereNull('digital_therapy_specific.location_id')
                    ->where(\DB::raw("CONCAT(DATE_FORMAT(date, '%Y-%m-%d'), ' ', end_time)"), '>=', Carbon::now()->toDateTimeString());
            }
            $getWellbeingSpecialist = $getWsLang = $getWellbeingSpecialist->where('users_services.service_id', $serviceSubCategory->id)
            ...
            // Display language dropdown for portal
            $getWsLang = $getWsLang->distinct()->get();
            $lang      = [];
            foreach ($getWsLang as $value) {
                if (!empty($value['language'])) {
                    $languageIds  = explode(',', $value['language']);
                    $languageList = config('zevolifesettings.userLanguage');
                    foreach ($languageIds as $langId) {
                        $lang[$langId]['id']    = (int) $langId;
                        $lang[$langId]['title'] = $languageList[$langId];
                    }
                }
            }
            // Filter by language
            if (!empty($request->lang)) {
                $langs = explode(',', $request->lang);
                $getWellbeingSpecialist = $getWellbeingSpecialist
                    ->where('users_services.service_id', $serviceSubCategory->id)
                    ->where(function($query) use($langs) {
                    foreach($langs as $lang) {
                        $query->orWhereRaw('FIND_IN_SET(?, ws_user.language)', [$lang]);
                    }
                });
            }
            $getWellbeingSpecialist = $getWellbeingSpecialist->distinct()->get();
    ...
    public function realTimeUIElementData(Request $request, User $user)
    {
        ...
        $healthCoachUnavailable = $combinedAvailability = $specificAvailabilities = [];
        try {
            $data      = array();
            ...
            if (!empty($digitalTherapyDetails)) {
                ...
                if (($setHoursBy == 2 && $setAvailabilityBy == 1) || ($setHoursBy == 1 && $setAvailabilityBy == 1)) {
                    ...
                    $specificAvailabilities = DigitalTherapySpecific::select(
                        \DB::raw("CONCAT(DATE_FORMAT(date, '%Y-%m-%d'), ' ', start_time) AS from_date"),
                        \DB::raw("CONCAT(DATE_FORMAT(date, '%Y-%m-%d'), ' ', end_time) AS to_date")
                    )
                        ...
                        ->get()
                        ->toArray();
                    $queryPeriod = generateQueryPeriod($combinedAvailability, $startTime, $endTime, $appTimezone, $digitalTherapySlot['timezone'], $healthCoachUnavailable, $duration, $specificAvailabilities);

                } elseif (($setHoursBy == 2 && $setAvailabilityBy == 2) || ($setHoursBy == 1 && $setAvailabilityBy == 2)) {
                    // Location - Specific And Company - Specific
    ...
    public function realTimeScheduling(Request $request, User $user)
    {
        ...
        try {
            ...
            $record                 = CronofySchedule::where('user_id', $loginUser->id)
                ->where('ws_id', $user->id)
                ->whereNotIn('status', ['canceled', 'short_canceled', 'completed', 'rescheduled', 'open'])
            ...
            if (empty($record)) {
                ...
                if (!empty($digitalTherapyDetails)) {
                    $setHoursBy        = $digitalTherapyDetails->set_hours_by;
                    $setAvailabilityBy = $digitalTherapyDetails->set_availability_by;
                    // Remove old Avaiability which set in cronofy
                    $this->cronofyRepository->availabilityRuleRemove($user);
                    ...
                    if (($setHoursBy == 2 && $setAvailabilityBy == 1) || ($setHoursBy == 1 && $setAvailabilityBy == 1)) {
                        $wsSlot = $user->healthCocahSlots()->select('day', 'start_time', 'end_time')->get()->toArray();
                        // Find union based on WBS and company availability
                        $combinedAvailability = alignedAvailability($digitalTherapySlot['data'], $wsSlot);
                        // Update availability
                         $this->cronofyRepository->updateAvailability($combinedAvailability, $user, $digitalTherapySlot['timezone'], false);
                        // Get all specific availability
                        $specificAvailabilities = DigitalTherapySpecific::select(
                            \DB::raw("CONCAT(DATE_FORMAT(date, '%Y-%m-%d'), ' ', start_time) AS from_date"),
                            \DB::raw("CONCAT(DATE_FORMAT(date, '%Y-%m-%d'), ' ', end_time) AS to_date")
                        )
                    ...
                    // Get Real time scheduling URL
                    $response = $this->cronofyRepository->realTimeScheduling($user, $company, $loginUser, $request->serviceId, $type, '', false, $healthCoachUnavailable, $digitalTherapySlot, $combinedAvailability, $specificAvailabilities);
            ...
            if (!empty($cronofySchedule)) {
                ...
                if (!empty($digitalTherapyDetails)) {
                    ...
                    if (($setHoursBy == 2 && $setAvailabilityBy == 1) || ($setHoursBy == 1 && $setAvailabilityBy == 1)) {
                        ...
                        // Find union based on company and WBS availability
                        $combinedAvailability = alignedAvailability($digitalTherapySlot['data'], $wsSlot);
                        // Update new availability in cronofy
                        $response = $this->cronofyRepository->updateAvailability($combinedAvailability, $wsUser, $digitalTherapySlot['timezone'], false);
                        // Find Specific Availability
                        $specificAvailabilities = DigitalTherapySpecific::select(
                            \DB::raw("CONCAT(DATE_FORMAT(date, '%Y-%m-%d'), ' ', start_time) AS from_date"),
                            \DB::raw("CONCAT(DATE_FORMAT(date, '%Y-%m-%d'), ' ', end_time) AS to_date")
                        )
                            ->where(\DB::raw("CONCAT(DATE_FORMAT(date, '%Y-%m-%d'), ' ', start_time)"), '>=', Carbon::now()->toDateString())
                            ->where('ws_id', $cronofySchedule->ws_id)
                            ->orderBy('date', 'DESC')
                            ->distinct()
                            ->get()
                            ->toArray();
                    ...
                    // Get Real time Scheduling url
                    $response = $this->cronofyRepository->realTimeScheduling($wsUser, $company, $loginUser, $serviceId, $type, $cronofySchedule->event_id, false, $healthCoachUnavailable, $digitalTherapySlot, $combinedAvailability, $specificAvailabilities);
    ...
    public function appointmentCancel(DeleteCronofyScheduleRequest $request, CronofySchedule $cronofySchedule)
    {
        ...
        try {
            ...
            if (!empty($cronofySchedule)) {
                ...
                if ($cronofySchedule->status == 'canceled' || $cronofySchedule->status == 'short_canceled') {
                    return $this->notFoundResponse('This session has been canceled.');
                }

                $duration = Carbon::parse($cronofySchedule->start_time)->diffInHours($nowInUTC);

                \DB::beginTransaction();
                if (!$cronofySchedule->is_group) {
                    $wsId     = $cronofySchedule->ws_id;
                    $eventId  = $cronofySchedule->event_id;
                    $this->cronofyRepository->cancelEvent($wsId, $eventId);
                    $status   = ($duration < 24) ? 'short_canceled' : 'canceled';
                    ...
                    if (empty($getConsentFormLogs)) {
                        ...
                        if ($xDeviceOS == config('zevolifesettings.PORTAL')) {
                            if ($consentCategory == 1) {
                                $deepLinkUri = __(config('zevolifesettings.portal_notification.consent_form_online'), [
                                    'id'   => (!empty($cronofySchedule->ws_id) ? $cronofySchedule->ws_id : 0),
                                    'type' => $consentCategory,
                                ]);
                            } else {
                ...
                $eventWsDate = Carbon::parse("{$cronofySchedule->start_time}", config('app.timezone'))->setTimezone($cronofySchedule->wellbeingSpecialist->timezone)->format('M d, Y');
                $eventWsTime = Carbon::parse("{$cronofySchedule->start_time}", config('app.timezone'))->setTimezone($cronofySchedule->wellbeingSpecialist->timezone)->format('h:i A');
                ...
                $sessionData = [
                    ...
                    'eventDate'       => $eventWsDate,
                    'eventTime'       => $eventWsTime,
                    'duration'        => $duration,
                ...
                $sessionDataForUsers = [
                    ...
                    'iCal'            => generateiCal([
                        ...
                        'inviteTitle' => trans('Cronofy.ical.title', [
                            'user_name' => $cronofySchedule->user->full_name,
            ...
            event(new DigitaltherapyExceptionHandlingEvent([
                ...
                'message'      => (string) trans('labels.common_title.something_wrong'),
                'company'      => $company,
                'wsDetails'    => $cronofySchedule->wellbeingSpecialist,
                'userDetails'  => $user,
                'errorDetails' => [],
            ]));
            return $this->internalErrorResponse(trans('api_labels.common.something_wrong_try_again'));
    ...
    public function createEventSlotDT(CreateEventSlogDTRequest $request, User $user)
    {
        ...
        try {
            ...
            if ($records) {
                ...
                $eventWsDate     = Carbon::parse("{$startDate}", config('app.timezone'))->setTimezone($newScheduleDetails->wellbeingSpecialist->timezone)->format('M d, Y');
                $eventWsTime     = Carbon::parse("{$startDate}", config('app.timezone'))->setTimezone($newScheduleDetails->wellbeingSpecialist->timezone)->format('h:i A');
                $sessionDataToWs = [
                    'company'       => (!empty($company) ? $company->id : null),
        ...
        try {
            ...
            if ($service->is_counselling) {
                $checkCounsellingSession = CronofySchedule::join('session_group_users', 'session_group_users.session_id', 'cronofy_schedule.id')
                    ...
                    ->whereNotIn('cronofy_schedule.status', ['canceled', 'short_canceled', 'rescheduled', 'open'])
            ...
            if ($sessionCompany != 0) {
                ...
                $scheduleCount = CronofySchedule::whereIn('user_id', $companyUsers)
                    ->whereNotIn('status', ['canceled', 'short_canceled', 'rescheduled', 'open'])
                    ->where(function ($query) use ($utcNow) {
