...
use App\Http\Controllers\API\V40\PortalHomeController as v40PortalHomeController;
...
class PortalHomeController extends v40PortalHomeController
{
    /**
    ...
    public function portalHome(Request $request)
    {
        try {
            ...
            $feedRecords->where(function (Builder $query) {
            ...
            $eventRecords = Event::select(
                ...
                ->whereRaw("(? BETWEEN CONVERT_TZ(CONCAT(event_booking_logs.booking_date, ' ', event_booking_logs.start_time), ? , ?) AND ADDTIME(CONVERT_TZ(CONCAT(event_booking_logs.booking_date, ' ', event_booking_logs.start_time), ? , ?), events.duration)) != 1 AND (capacity IS NOT NULL AND capacity > 0 AND ((SELECT IFNULL(COUNT(event_registered_users_logs.user_id), 0) FROM event_registered_users_logs WHERE event_registered_users_logs.is_cancelled = 0 AND event_registered_users_logs.event_booking_log_id = event_booking_logs.id) >= capacity)) != 1",[
                    $now,$appTimezone,$timezone,$appTimezone,$timezone
                ])
            ...
            if(!empty($user)){
                ...
                $contactUsDesc = "";
                if (!empty($brandingContactDetails) && !empty($brandingContactDetails->contact_us_description)) {
                    $contactUsDesc = $brandingContactDetails->contact_us_description;
                } elseif (!empty($brandingContactDetailsParent) && !empty($brandingContactDetailsParent->contact_us_description)) {
                    $contactUsDesc = $brandingContactDetailsParent->contact_us_description;
                }
                ...
                $brandingData['contactUsDescription']  =  $contactUsDesc;
                ...
                } elseif (!empty($brandingContactDetailsParent->getFirstMedia('contact_us_image')->name)) {
                ...
                } elseif (!empty($parentCompany->branding->portal_footer_text)) {
                ...
                } elseif (!empty($parentCompany->branding->portal_footer_header_text)) {
                ...
                } elseif (!empty($parentCompany->branding->portal_footer_json)) {
                ...
                } elseif (!empty($parentCompany->getFirstMedia('portal_footer_logo'))) {
                ...
                // Display Appointment details for parent and child companis
                if (!empty($userCompany->branding->appointment_title)) {
                    $brandingData['AppointmentTitle'] = $userCompany->branding->appointment_title;
                } elseif (!empty($parentCompany->branding->appointment_title)) {
                    $brandingData['AppointmentTitle'] = $parentCompany->branding->appointment_title;
                }

                if (!empty($userCompany->branding->appointment_description)) {
                    $brandingData['AppointmentDescription'] = $userCompany->branding->appointment_description;
                } elseif (!empty($parentCompany->branding->appointment_description)) {
                    $brandingData['AppointmentDescription'] = $parentCompany->branding->appointment_description;
                }

                if (!empty($userCompany->getFirstMedia('appointment_image'))) {
                    $appointmentImage = $userCompany->getMediaData('appointment_image', ['w' => 800, 'h' => 800]);
                } elseif (!empty($parentCompany->getFirstMedia('appointment_image'))) {
                    $appointmentImage = $parentCompany->getMediaData('appointment_image', ['w' => 800, 'h' => 800]);
                } else {
                    $appointmentImage['width']   = config('zevolifesettings.imageConversions.company.appointment_image.width');
                    $appointmentImage['height']  = config('zevolifesettings.imageConversions.company.appointment_image.height');
                    $appointmentImage['url']     = config('zevolifesettings.fallback_image_url.company.appointment_image_default');
                }
                $brandingData['AppointmentImage'] = $appointmentImage;
                ...
                // Set the case manager link
                $caseManagerCompanyCode = config('zevolifesettings.is_case_manager_access_for_portal.' . $appEnvironment);
                $caseManagerFlag        = false;
                $caseManagerLink        = '';
                if (in_array($userCompany->code, $caseManagerCompanyCode)) {
                    $caseManagerFlag      = true;
                    $caseManagerLink      = config('zevolifesettings.case_manager_access_link_for_portal.' . $appEnvironment)[$userCompany->code];
                }
                $brandingData['caseManagerFlag'] = $caseManagerFlag;
                $brandingData['caseManagerLink'] = $caseManagerLink;

                // Set the Survicate flag false only for production
                $isSurvicateEnabled = false;
                $survicateAccessCompanyCode = config('zevolifesettings.is_survicate_access_for_portal.' . $appEnvironment);
                if (in_array($userCompany->code, $survicateAccessCompanyCode)) {
                    $isSurvicateEnabled = true;
                }
                $brandingData['isSurvicateEnabled'] = $isSurvicateEnabled;
