...
use App\Http\Collections\V26\BadgeListDetailsCollection;
use App\Http\Controllers\API\V26\BadgeController as v26BadgeController;
use App\Http\Resources\V26\BadgeDetailResource;
...
class BadgeController extends v26BadgeController
{
    ...
    public function active(Request $request)
    {
        try {
            ...
            $data['generalBadge'] = Badge::whereIn('type', ['general', 'daily'])
                ...
                ->selectRaw("(SELECT count(id) FROM badge_user WHERE badge_id = badges.id AND user_id = ? AND STATUS = 'Active') AS assignCount",[
                    $user->id
                ])
            ...
            if ($checkAccess) {
                $challengeBadge->orWhere('type', 'ongoing');
            }
            $challengeBadge = $challengeBadge->select(
                'id',
                'title',
                'type'
            )
