...
namespace App\Http\Controllers\Api\V29\Auth;

use App\Http\Collections\V28\AppSettingCollection;
use App\Http\Controllers\API\V28\Auth\LoginController as v28LoginController;
use App\Http\Controllers\Controller;
use App\Http\Requests\Api\V1\EmailRequest;
use App\Http\Requests\Api\V1\LoginRequest;
use App\Http\Resources\V20\UserResource;
 ...
 */
class LoginController extends v28LoginController
{
    ...
    public function attemptLogin(Request $request)
    {
        ...
        if (!empty($request->get('email')) && !empty($request->get('socialId'))) {
            ...
            // else {
            //     $this->guard()->setTTL(40320);
            // }
            if ($request->header('X-Device-Os') == config('zevolifesettings.PORTAL')) {
                return $this->guard()->attempt($this->credentials($request));
            } else {
                $user1 = User::where('email', $request->get('email'))->first();//->where('single_use_code', $request->get('singleUseCode'))->first();
    ...
    {
        $xDeviceOs      = strtolower($request->header('X-Device-Os', ""));
        if ($xDeviceOs != config('zevolifesettings.PORTAL') && empty($request->get('socialId'))) {
    ...
    public function sendSingleUseCode(EmailRequest $request, $fromApi = 'login')
    {
        try {
            ...
            if ($fromApi == 'login') {
                ...
                if ($companyDetails) {
                    if (($xDeviceOs == config('zevolifesettings.IOS') || $xDeviceOs == config('zevolifesettings.ANDROID')) && !$companyDetails->allow_app ) {
                        return $this->notaccessFailedResponse(\trans('api_labels.auth.not_access_app'));
                    }
                } else {
                    $message = ($xDeviceOs != config('zevolifesettings.PORTAL')) ? \trans('api_labels.auth.not_access_app') : \trans('api_labels.auth.not_access_portal');
            ...
            if ($record) {
                ...
                $otpValidTill        = ($isResent == 1  ? '1 minute' : '15 minutes');
    ...
    public function verifySingleUseCode(Request $request)
    {
        try {
            ...
            if (empty($userData)) {
                throw ValidationException::withMessages([
                    'singleUseCode' => trans('api_labels.auth.invalid_otp'),
                ])->status(422);
            } else {
                if ($userData->is_resent == 1) {
                    $codeExpired = $query
                    ->whereRaw("DATE_ADD(updated_at, INTERVAL '1' MINUTE) >= ?", $now)
                    ->first();
                } else {
                    $codeExpired = $query
                    ->whereRaw("DATE_ADD(created_at, INTERVAL '15' MINUTE) >= ?", $now)
                    ->first();
                }
