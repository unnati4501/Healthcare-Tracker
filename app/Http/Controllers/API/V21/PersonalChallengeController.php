...
use App\Http\Collections\V21\PersonalChallengeHistoryCollection;
use App\Http\Collections\V21\PersonalChallengeListCollection;
use App\Http\Controllers\API\V6\PersonalChallengeController as v6PersonalChallengeController;
...
use App\Http\Resources\V21\PersonalChallengeDetailResource;
...
class PersonalChallengeController extends v6PersonalChallengeController
{
    ...
    public function explorePersonalChallenges(Request $request)
    {
        try {
            ...
            $exploreChallengeData = PersonalChallenge::with(['personalChallengeUsers'])
                ->join("personal_challenge_users", function ($join) use ($user) {
                ...
                ->where(function ($query) use ($company, $user) {
                    $query->where('personal_challenges.company_id', null)
                        ->orWhere('personal_challenges.creator_id', $user->id);
                })
    ...
    /**
     * delete personal challenge from app
     *
    ...
    public function delete(PersonalChallenge $personalChallenge)
    {
        try {
            ...
            // check user is creater of personal challenge or not if user is not creater of personal challenge then not allow to perform operation
            if ($personalChallenge->creator_id != $loggedInUser->getKey()) {
                return $this->notFoundResponse("You are not authorized to delete this personal challenge");
            }

            \DB::beginTransaction();
            $personalChallenge->clearMediaCollection('logo');
            $personalChallenge->delete();
            \DB::commit();

            return $this->successResponse([], trans('api_messages.personalChallenge.delete'));
        } catch (\Exception $e) {
            \DB::rollback();
            report($e);
            return $this->internalErrorResponse(trans('labels.common_title.something_wrong'));
