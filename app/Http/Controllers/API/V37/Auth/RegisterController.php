...
namespace App\Http\Controllers\Api\V37\Auth;

use App\Http\Controllers\API\V36\Auth\RegisterController as v36RegisterController;
 ...
 */
class RegisterController extends v36RegisterController
{
    ...
    public function register(RegisterRequest $request)
    {
        try {
            ...
            if ($xDeviceOs == config('zevolifesettings.PORTAL')) {
                $department = Department::find($request->department_id, ['id']);
                ...
                $team = $department->teams()
                    ->whereHas('teamlocation', function ($query) use ($locationId) {
                        $query->where('company_locations.id', $locationId);
                    })
                    ->whereNotIn('teams.id', $chInvolvedTeams)
                    ->get();
                if ($team->isNotEmpty()) {
                    $team = $team->random(1)->first();
                } else {
                    return $this->invalidResponse([
                        'team_id' => ['No team available for selected company!'],
            ...
            if (!empty($user)) {
                if ($xDeviceOs != config('zevolifesettings.PORTAL') && $user->can_access_app) {
                    throw \Illuminate\Validation\ValidationException::withMessages([
                        'email' => trans('api_labels.auth.email_exist'),
                    ])->status(422);
                    ...
                    if (empty($request->get('socialId'))) {
                        if (!empty($request->get('password'))) { //$xDeviceOs == config('zevolifesettings.PORTAL') &&
                    ...
                    $userData = [
                        ...
                        'can_access_app'    => (isset($userCompany->allow_app) && $userCompany->allow_app),
                        'can_access_portal' => (isset($userCompany->allow_portal) && $userCompany->allow_portal),
                    ];
                ...
                if ($company->has_domain) {
                    list(, $domain) = \explode('@', strtolower($request->input('email')));
                ...
                $userData = [
                    ...
                    'last_activity_at'         => \now(config('app.timezone')),
                    'start_date'               => $company->subscription_start_date,
                    'consent_privacy'          => $request->input('privacyConsent') ?? 1,
                    'consent_terms_conditions' => $request->input('termsConditions') ?? 1,
                    'consent_data'             => $request->input('dataConsent') ?? 1,
                ];
            ...
            if (!empty($user) && $request->hasFile('profileImage')) {
            ...
            $version =  getApiVersion();//config('zevolifesettings.version.api_version');
            // To call internal login API - creates request object
            $loginRequest = Request::create('api/v' . $version . '/auth/login', 'POST', $headers, $payload);
