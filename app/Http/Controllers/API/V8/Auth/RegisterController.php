...
namespace App\Http\Controllers\Api\V8\Auth;

use App\Http\Controllers\API\V6\Auth\RegisterController as v6RegisterController;
 ...
 */
class RegisterController extends v6RegisterController
{
    ...
    public function register(RegisterRequest $request)
    {
        try {
            ...
            $company = \App\Models\Company::where('code', $request->input('companyCode'))->first();
            ...
            if (!empty($user)) {
                ...
                /*if (!empty($request->input('expertiseLevel'))) {
                foreach ($request->input('expertiseLevel') as $key => $value) {
                $category = \App\Models\Category::find($value['categoryId']);

                if (!empty($category)) {
                $user->expertiseLevels()->attach($category, ['expertise_level' => strtolower($value['level'])]);
                }
                }
                }*/
                ...
                $userGoalData['calories'] = ($request->input('gender') == "male") ? 2500 : 2000;
