...
use App\Http\Collections\V22\EventListCollection;
use App\Http\Controllers\API\V21\EventController as v21EventController;
...
class EventController extends v21EventController
{
    /**
    ...
    public function index(Request $request, $type = 'registered')
    {
        try {
            ...
            if ($eventRecords->count() > 0) {
                $events  = new EventListCollection($eventRecords, (($type == "booked" && !empty($sessionDetails)) ? $sessionDetails : null));
                $message = 'Events Retrieved Successfully';
            } else {
                $events = ['data' => []];
                if ($type == "booked" && !empty($sessionDetails)) {
