...
use App\Http\Controllers\API\V33\MeditationController as v33MeditationController;
...
class MeditationController extends v33MeditationController
{
    ...
    public function markAsCompleted(Request $request, MeditationTrack $meditation)
    {
        try {
            ...
            UpdatePointContentActivities('meditation', $meditation->id, $user->getKey(), 'completed', true);
    ...
    public function likeUnlikeTrack(Request $request, MeditationTrack $meditation)
    {
        try {
            ...
            $pivotExsisting = $meditation
                ...
                ->first();
            $extraPoint = !is_null($meditation->tag_id) ;
            if (!empty($pivotExsisting)) {
                ...
                if ($liked == 1) {
                    ...
                    UpdatePointContentActivities('meditation', $meditation->id, $user->id, 'like', false, $extraPoint);
                }
            } else {
                UpdatePointContentActivities('meditation', $meditation->id, $user->id, 'like', false, $extraPoint);
