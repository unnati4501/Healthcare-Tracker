...
use App\Http\Collections\V24\EventListCollection;
use App\Http\Controllers\API\V24\EventController as v24EventController;
use App\Http\Resources\V20\EventDetailsResource;
use App\Http\Resources\V23\EAPSessionResource;
...
class EventController extends v24EventController
{
    /**
    ...
    public function index(Request $request, $type = 'registered')
    {
        try {
            ...
            $sessionDetails = $user->bookedSessions()
                ->select(
                    'eap_calendly.id',
                    'eap_calendly.name',
                ...
                ->orderBy('eap_calendly.start_time');
            ...
            if (!empty($request->month)) {
                ...
                $sessionDetails
                    ->whereRaw("TIMESTAMP(eap_calendly.start_time) BETWEEN '{$fromdate}' AND '{$todate}'");
            }
            ...
            if ($checkAlreadyRegisterd) {
                ...
                if (!$cancelledStatus) {
                    ...
                    event(new SendEventCancelledEvent($user, [
                        ...
                        "message" => "Hi {$user->full_name},<br/> This is to notify you that your Registration for {$event->name} on {$displayStartTime} has been cancelled.",
                        'iCal'    => generateiCal([
                            'uid'           => $uid,
                            'appName'       => config('app.name'),
                            'inviteTitle'   => $event->name,
