...
use App\Http\Collections\V12\WellbeingScoreCategoryCollection;
...
class PortalHomeController extends Controller
{
    ...
    /**
     * Home page wellbeing score for portal
     *
     * @return \Illuminate\Http\JsonResponse
     */
    public function wellbeingScore(Request $request)
    {
        try {
            $user    = $this->user();
            $company = $user->company()->first();
            $data    = array();
            // $zc_survey_max_score_value = config('zevolifesettings.zc_survey_max_score_value');

            // Get all over score of survey
            $allOverSurveyResponse = ZcSurveyResponse::select(\DB::raw("FORMAT(IFNULL(((IFNULL(SUM(zc_survey_responses.score), 0) * 100) / IFNULL(SUM(zc_survey_responses.max_score), 0)), 0), 2) AS percentage"))
                ->where('zc_survey_responses.user_id', $user->id)
                ->groupBy('zc_survey_responses.user_id')
                ->first();

            // Get score category wise of survey
            $surveyCategoryResponse = ZcSurveyResponse::select('zc_categories.id', 'zc_survey_responses.category_id', 'zc_categories.display_name', \DB::raw("FORMAT(IFNULL(((IFNULL(SUM(zc_survey_responses.score), 0) * 100) / IFNULL(SUM(zc_survey_responses.max_score), 0)), 0), 2) AS percentage"))
                ->leftJoin('zc_survey_log', 'zc_survey_log.id', '=', 'zc_survey_responses.survey_log_id')
            ...
            $percentage           = isset($allOverSurveyResponse->percentage) ? $allOverSurveyResponse->percentage : 0;
            $data['allOverScore'] = $percentage;
            $data['allOverColor'] = portalSurveyColorCode($percentage);

            $data['categoryScore'] = new WellbeingScoreCategoryCollection($surveyCategoryResponse, true);
