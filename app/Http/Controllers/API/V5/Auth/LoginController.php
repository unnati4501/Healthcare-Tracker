...
use App\Http\Collections\V5\AppSettingCollection;
use App\Http\Controllers\API\V4\Auth\LoginController as v4LoginController;
 ...
 */
class LoginController extends v4LoginController
{
    ...
    public function authenticated(Request $request, $user, $token): JsonResponse
    {
        ...
        $lastActivityAt = Carbon::parse($user->last_activity_at, config('app.timezone'))->setTimezone($user->timezone)->todatetimeString();
    ...
    public function appLaunch(Request $request)
    {
        try {
            ...
            return $this->successResponse(($records->count() > 0) ? new AppSettingCollection($records, $forceUpdateCount, strtolower($deviceOs), $lastActivityAt) : ['data' => []], 'Data recevied successfully.');
        } catch (\Exception $e) {
            report($e);
            return $this->internalErrorResponse(trans('labels.common_title.something_wrong'));
        }
