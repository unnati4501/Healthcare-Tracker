...
use App\Http\Collections\V1\CategoryWiseCourseCollection;
use App\Http\Collections\V1\CoachReviewCollection;
use App\Http\Collections\V1\CompletedCourseCollection;
use App\Http\Collections\V1\CourceCoachDetailsCollection;
...
use App\Http\Collections\V1\NotStartedCourseCollection;
use App\Http\Collections\V1\OnGoingCourseCollection;
use App\Http\Controllers\Controller;
use App\Http\Requests\Api\V1\CourseCoachReviewRequest;
use App\Http\Resources\V1\CoachDetailsByCoachResource;
use App\Http\Resources\V1\CourceBenefitInstructionResource;
use App\Http\Resources\V1\CourseDetailsResource;
use App\Http\Resources\V1\CourseLessonDetailsResource;
...
class CourseController extends Controller
{
        ...
        try {
            ...
            if (!empty($courseIds)) {
                ...
                $categoryWiseCourseData = Course::whereIn("courses.id", $courseIds)
                    ...
                    ->leftJoin(DB::raw("(SELECT course_lessions.course_id,  COUNT(DISTINCT course_lessions.course_week_id) AS moduleCount FROM course_lessions WHERE course_lessions.is_default = FALSE GROUP BY course_lessions.course_id) as courseModule"), "courses.id", "=", "courseModule.course_id")
        ...
        try {
            ...
            if (!empty($pivotExsisting)) {
                ...
                if (!$is_joined) {
                    ...
                    $firstLesstion = $course->courseLessions()->where('is_default', false)->orderBy('course_week_id', 'ASC')->orderBy('id', 'ASC')->first();

                    if (!empty($firstLesstion)) {
                        $allPriorWeeks = $course->courseWeeks()->where('course_weeks.id', '<', $firstLesstion->course_week_id)->orderBy('course_weeks.id', 'ASC')->get();
    ...
    public function lessonStatusChange(Request $request, CourseLession $courseLession)
    {
        try {
            ...
            $totalLessionCountOfWeek = $courseWeek->courseLessions()->where('is_default', false)->count();
            ...
            if (!empty($pivotExsisting) && $status == 'completed') {
                ...
                if (!empty($thisWerkLogOfUser) && $thisWerkLogOfUser->pivot->status == 'completed') {
                    ...
                    $nextWeek = $course->courseWeeks()->where('is_default', false)->where('course_weeks.id', '>', $courseWeek->id)->orderBy('course_weeks.id', 'ASC')->first();

                    if (!empty($nextWeek)) {
                        $lessionCountOfNextWeek = $nextWeek->courseLessions()->where('is_default', false)->count();
                        ...
                        do {
                            if ($lessionCountOfNextWeek == 0) {
                                ...
                                $nextWeek = $course->courseWeeks()->where('is_default', false)->where('course_weeks.id', '>', $nextWeek->id)->orderBy('course_weeks.id', 'ASC')->first();
                        ...
                        $nextLesstion = $nextWeek->courseLessions()->where('is_default', false)->whereNotIn('id', $allUnlockedIdsOfNextWeek)->orderBy('id', 'ASC')->first();
                    }
                    ...
                    $nextLesstion = $courseWeek->courseLessions()->where('is_default', false)->whereNotIn('id', $allUnlockedIdsOfThisWeek)->orderBy('id', 'ASC')->first();
                }
                ...
                $totalLessionsOfCourse = $course->courseLessions()->where('is_default', false)->count();
