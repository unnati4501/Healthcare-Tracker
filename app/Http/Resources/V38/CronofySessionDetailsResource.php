...
class CronofySessionDetailsResource extends JsonResource
{
    ...
    public function toArray($request)
    {
        ...
        $cancellationDetails = [];
        if ($this->status == 'canceled' || $this->status == 'short_canceled') {
        ...
        $allowBook = (($this->status != 'canceled' || $this->status == 'short_canceled') ? !$this->hasUpComingSession : false);
        ...
        return [
            ...
            'status'                     => $status,
            'cancellationDetails'        => $this->when(($this->status == 'canceled' || $this->status == 'short_canceled'), $cancellationDetails),
            ...
            'consentType'                => ($isOffline && !$isOnline ? 2 : 1) // 1 => online 2 => offline
        ];
    }
}
