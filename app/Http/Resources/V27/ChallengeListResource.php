...
class ChallengeListResource extends JsonResource
{
    ...
    public function toArray($request)
    {
        ...
        $currentDateTimeString = now($user->timezone);

        if ($currentDateTime >= $startDate) {
            $isStarted = true;
        }

        if ($currentDateTimeString->greaterThan($endDate)) {
            $isCompleted = true;
        }
        ...
        return [
            ...
            'rank'                  => $this->when($isCompleted , ((isset($pointsRecord) && !empty($pointsRecord->rank)) ? $pointsRecord->rank : 0)),
        ];
    }
}
