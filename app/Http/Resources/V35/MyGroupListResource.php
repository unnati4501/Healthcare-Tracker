    ...
    public function toArray($request)
    {
        ...
        $members = $this->members()
            ...
            if (!empty($this->created_by && $this->created_by == 'User')) {
                $members = $members->whereRaw("accept_decline_status != 2");
            }
        ...
        $subCategory = ($isMember || $this->created_by == 'User') ? $this->subcategory->name : 'Public';

        if ($this->subcategory->short_name == 'challenge') {
            $groupIcon = getStaticGroupIconUrl('challenge');
        } elseif ($this->subcategory->short_name == 'masterclass') {
            $groupIcon = getStaticGroupIconUrl('masterclass');
        } else {
            $groupIcon = getStaticGroupIconUrl('standard');
        }
        ...
        return [
            ...
            'isNotificationMute'   => (!empty($loginUserData) && $loginUserData->pivot->notification_muted) ? true : false,
