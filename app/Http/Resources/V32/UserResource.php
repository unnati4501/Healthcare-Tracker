...
class UserResource extends JsonResource
{
    ...
    public function toArray($request)
    {
        ...
        $goalMappedCount          = (($goalRecords > 0) ? true : false);
        $appSlide                 = (AppSlide::where('type', $type)->count() > 0) ? true : false;
        $appTheme                 = [];
        $csatAvailable            = false;
        ...
        // enable_event flag
        $eventEnable = (!empty($userCompany) ? $userCompany->enable_event : false);

        // hide content flag
        // get the parent company data
        if (!is_null($userCompany->parent_id)) {
            $rsaCompnayData = Company::select('hide_content')->where('id', $userCompany->parent_id)->first();
            $hideContent    = (!empty($rsaCompnayData) ? (bool) $rsaCompnayData->hide_content : false);
        } else {
            $hideContent = (!empty($userCompany) ? (bool) $userCompany->hide_content : false);
        }
        ...
        // check survey for all is enabled/disabled
        ...
        if ($zcsurveylog) {
            $zcSurveyUserResponse = ZcSurveyResponse::where('user_id', $this->id)
                ->where('company_id', $userCompany->id)
                ->where('survey_log_id', $zcsurveylog->id)
                ->count();

            // dd($zcSurveyUserResponse, $this->start_date, $todayDate);
            if ($zcSurveyUserResponse >= 0 && $this->start_date <= $todayDate) {
                $zcSurveyUserLog = ZcSurveyUserLog::select('id', 'survey_submitted_at')->where('user_id', $this->id)
                    ->where('survey_log_id', $zcsurveylog->id)
                    ->first();
                if ($zcSurveyUserResponse == 0 && $zcsurveylog->survey_to_all  && (!is_null($zcSurveyUserLog))) {
                    $surveyDisplay   = true;
                } else {
                    $surveyDisplay   = false;
                }
                $surveyIdDisplay = $surveyDisplay;//(isset($zcSurveyUserLog)) ? true : false;
            } else {
                // here if survey_to_all is set to false and user isn't present in survey user logs then prevent user to avail the survey as user isn't selected in survey config
                $surveyDisplay   = $zcsurveylog->survey_to_all;
                $surveyIdDisplay = $zcsurveylog->survey_to_all;
            }
        }
        ...
        if ($xDeviceOs == config('zevolifesettings.PORTAL')) {
            $surveySubmittedCount = ZcSurveyResponse::where('user_id', $this->id)
                ->where('company_id', $userCompany->id)
                ->count();
            $wellbeingSurveySubmitted = ($surveySubmittedCount > 0) ? true : false;
            ...
            $csatAvailable = (!empty($userNpsLogs)) ? true : false;
        }
        ...
        if(!empty($userCompany)){
            $checkDTAccess  = getCompanyPlanAccess($this, 'digital-therapy');
        }
        return [
            'id'                       => $this->id,
            'name'                     => $this->full_name,
            ...
            'healthScoreAvailable'     => (!empty($this->hs_show_banner) && $this->hs_show_banner ) ? true : false,
            ...
            'surveyLogId'              => (($surveyIdDisplay ) ? $zcsurveylog->id : 0),
            'eapSetting'               => new GethelpResource($this),
            'eapTab'                   => $checkDTAccess,
            ...
            'hideContent'              => $this->when($xDeviceOs == config('zevolifesettings.PORTAL'), $hideContent),
            'socialId'                 => $this->when(!empty($this->social_id), $this->social_id),
            'socialType'               => $this->when(!empty($this->social_type), (int) $this->social_type),
            'eventEnable'              => $this->when($xDeviceOs != config('zevolifesettings.PORTAL'), $eventEnable),
        ];
    }
}
