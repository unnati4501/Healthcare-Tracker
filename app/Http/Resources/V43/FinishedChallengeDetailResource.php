...
use App\Http\Collections\V32\ChallengeOtherUserMapCollection;
use App\Http\Collections\V33\ContentChallengeActivityCollection;
...
class FinishedChallengeDetailResource extends JsonResource
{
    ...
    public function toArray($request)
    {
        ...
        $badgeData = Badge::select(
            ...
            ->whereraw("IF ((badges.type !=  'challenge' and (SELECT count(id) FROM badge_user WHERE badge_id = badges.id AND user_id = ? AND model_id = ? AND STATUS = 'Active') = 0), DATE_ADD(challenges.start_date, INTERVAL challenge_ongoing_badges.in_days DAY) > ?, challenges.start_date IS NOT NULL) ",[
                $user->id,$this->id,$currentAppDateTime
            ]);
        ...
        foreach ($challengeRulesData as $outer => $rule) {
            ...
            foreach ($daysRange as $inner => $day) {
                ...
                $points = round($count / $this->pointCalcRules[$point], 1);*/
                    ...
                    $count = 0;
                    if ($this->challenge_type == 'individual') {
                        $count = $this->contentChallengePointHistory()
                        ->where(\DB::raw("DATE(CONVERT_TZ(content_challenge_point_history.log_date, '{$appTimezone}', '{$userTimezone}'))"), '=', $daysData['date'])
                        ...
                        if ($this->finished) {
                            ...
                            foreach ($teamData as $key => $value) {
                                $count += $value->getContentPointHistory($this, $daysData['date'], $appTimezone, $userTimezone);
                            }
                        }
                    }
                }
        ...
        if (!empty($this->challenge_category_id) && !empty($challenge_rule_description[$this->challenge_type]) && !empty($challenge_rule_description[$this->challenge_type][$this->challenge_category_id])) {
        ...
        if (!empty($mapChallenge)) {
            ...
            // Multiple User in Map Challenge

            // Top rank User details
            $topRankId = null;
            $topRank   = $this->challengeHistorySteps()->join('challenge_wise_user_ponits', 'challenge_wise_user_ponits.challenge_id', '=', 'freezed_challenge_steps.challenge_id')->join('users', 'users.id', '=', 'freezed_challenge_steps.user_id')->select('users.id', 'users.first_name', 'users.last_name', 'challenge_wise_user_ponits.rank', 'challenge_wise_user_ponits.rank', 'freezed_challenge_steps.steps', 'freezed_challenge_steps.id AS freezedChallegeId', DB::raw("'{$this->map_id}' AS map_id"))->whereNotIn('freezed_challenge_steps.user_id', [$user->id])->orderbyDesc('freezed_challenge_steps.steps')->first();
            if (!empty($topRank)) {
                $otherUsers[] = $topRank;
                $topRankId    = $topRank->freezedChallegeId;
            }
            // Prefix User details
            $prefix = $this->challengeHistorySteps()->join('challenge_wise_user_ponits', 'challenge_wise_user_ponits.challenge_id', '=', 'freezed_challenge_steps.challenge_id')->join('users', 'users.id', '=', 'freezed_challenge_steps.user_id')->select('users.id', 'users.first_name', 'users.last_name', 'challenge_wise_user_ponits.rank', 'challenge_wise_user_ponits.rank', 'freezed_challenge_steps.steps', DB::raw("'{$this->map_id}' AS map_id"))->where('freezed_challenge_steps.steps', '>=', $totalCount)->whereNotIn('freezed_challenge_steps.user_id', [$user->id])->whereNotIn('freezed_challenge_steps.id', [$topRankId])->orderbyDesc('challenge_wise_user_ponits.id')->first();
            if (!empty($prefix)) {
                $otherUsers[] = $prefix;
            }
            // Postfix User details
            $postfix = $this->challengeHistorySteps()->join('challenge_wise_user_ponits', 'challenge_wise_user_ponits.challenge_id', '=', 'freezed_challenge_steps.challenge_id')->where('freezed_challenge_steps.steps', '<=', $totalCount)->join('users', 'users.id', '=', 'freezed_challenge_steps.user_id')->select('users.id', 'users.first_name', 'users.last_name', 'challenge_wise_user_ponits.rank', 'challenge_wise_user_ponits.rank', 'freezed_challenge_steps.steps', DB::raw("'{$this->map_id}' AS map_id"))->whereNotIn('freezed_challenge_steps.user_id', [$user->id])->whereNotIn('freezed_challenge_steps.id', [$topRankId])->orderbyDesc('challenge_wise_user_ponits.id')->first();
        ...
        $contentChallenge = $challengeRules = $contentActivity = [];
        if ($rule->short_name == 'content') {
            $challengeRules = $this->challengeRules()->select('content_challenge_ids')->first();
            if (!empty($challengeRules)) {
                $contentChallengeIds = explode(',', $challengeRules->content_challenge_ids);
                $contentChallenge    = ContentChallenge::whereIn('id', $contentChallengeIds)->get();
                $contentActivity     = new ContentChallengeActivityCollection($contentChallenge);
            }
        }
        ...
        $return = [
            ...
            'contentActivity'       => $this->when(!empty($contentActivity), $contentActivity),
            ...
            'mapLocations'          => $this->when(!empty($mapChallenge), [
                ...
                'otherUsers'        => (!empty($otherUsers)) ? (new ChallengeOtherUserMapCollection($otherUsers)) : [],
    ...
    private function getLeaderboard($challenge)
    {
        ...
        if ($challengeParticipantsWithPoints) {
            ...
            $participant['image'] = (!is_null($userRecord)) ? $userRecord->getMediaData('logo', ['w' => 320, 'h' => 320]) : config('zevolifesettings.fallback_image_url.user.logo');
        }
