...
class FinishedChallengeDetailResourceNew extends JsonResource
{
    ...
    public function toArray($request)
    {
        ...
        foreach ($challengeRulesData as $outer => $rule) {
                            ...
                            if (!empty($freezedTeamUserList)) {
                                $count = $this->challengeHistorySteps()
                                    ->whereRaw("DATE(CONVERT_TZ(freezed_challenge_steps.log_date, ?, ?)) = ?",[
                                        $appTimezone,$userTimezone,$daysData['date']
                                    ])
            ...
            if ($this->challenge_type == 'team') {
                ...
                if ($this->challengeCatShortName == 'streak') {
                    $challengeRulesArray['remaining'] = (float) number_format(($targetDayValueForStreak - $currentDayValueForStreak), 1, '.', '');
                } elseif ($this->challengeCatShortName == 'most') {
        ...
        if (!empty($this->freezed_data_at) && !$isCompleted) {
            $description .= "<br/><br/><b>Last points updated at " . Carbon::parse($this->freezed_data_at, $appTimezone)->setTimezone($userTimezone)->format('d/m/Y h:i A') . "<br/><br/>Next points update will be on " . Carbon::parse($this->freezed_data_at, $appTimezone)->setTimezone($userTimezone)->addHours(2)->format('d/m/Y h:i A') . "</b>";
        }
