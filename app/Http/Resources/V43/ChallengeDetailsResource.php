...
class ChallengeDetailsResource extends JsonResource
{
    ...
    public function toArray($request)
    {
        ...
        ->selectRaw('? as userTimezone',[
            $user->timezone
        ])
        ->selectRaw("(SELECT id FROM badge_user WHERE badge_id = badges.id AND user_id = ? AND model_id = ? AND STATUS = 'Active') AS badgeUserId",[
            $user->id,$this->id
        ])
        ->selectRaw("(SELECT count(id) FROM badge_user WHERE badge_id = badges.id AND user_id = ? AND model_id = ? AND STATUS = 'Active') AS assignCount",[
            $user->id,$this->id
        ])
