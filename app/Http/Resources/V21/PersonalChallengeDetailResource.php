...
class PersonalChallengeDetailResource extends JsonResource
{
    ...
    public function toArray($request)
    {
        ...
        $recursiveCount           = 0;
        if ($isJoined) {
            $reminderAt               = Carbon::parse($userData->pivot->reminder_at, config('app.timezone'))->toTimeString();
            $challengeStartReminderAt = $challengeStartDate->toDateString() . ' ' . $reminderAt;
            $challengeStartReminderAt = Carbon::parse($challengeStartReminderAt);
        ...
        $returnArray = [
            ...
            'recursiveCount'        => $this->when($isJoined == true, $recursiveCount),
            'timerData'             => $this->when($isJoined == true, $timerData),
            'challengeStartDate'    => $this->when($isJoined == true, $challengeStartDate->toAtomString()),
            'challengeEndDate'      => $this->when($isJoined == true, $challengeEndDate->toAtomString()),
            'challengeReminderAt'   => $this->when($isJoined == true, $challengeStartReminderAt->toAtomString()),
            'durationInHumanFormat' => $this->when($isJoined == true, $durationInHumanFormat),
