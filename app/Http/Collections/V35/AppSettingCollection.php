...
namespace App\Http\Collections\V35;
    ...
    public function toArray($request)
    {
                ...
                if ($setting->type == 'file') {
                    ...
                    if ($setting->key == 'app_theme') {
                        if (Schema::hasTable('app_themes')) {
                            AppTheme::where('slug', $setting->value)->first();
                            $path     = "";
        ...
        if ($request->header('X-Device-Os') == 'portal') {
            ...
            if (!empty($branding)) {
                ...
                if (!empty($parentCompany->branding->portal_footer_text)) {
                    $brandingData['portalFooterText'] = $parentCompany->branding->portal_footer_text;
                }

                //Display header text on footer screen
                if (!empty($parentCompany->branding->portal_footer_header_text)) {
                    $brandingData['portalFooterHeaderText'] = $parentCompany->branding->portal_footer_header_text;
                }

                if (!empty($parentCompany->branding->portal_footer_json)) {
                    $brandingData['portalFooterJson'] = json_decode($parentCompany->branding->portal_footer_json, true);
                }

                $brandingData['portalFooterLogo']    = $company->getMediaData('portal_footer_logo', ['w' => 180, 'h' => 60]);
