...
class EditWebinarRequest extends FormRequest
{
    /**
    ...
    {
        return access()->allow('edit-webinar');
    }
    ...
    public function rules()
    {
        ...
        $trackMax           = config('zevolifesettings.fileSizeValidations.webinar.track', (100 * 1024));
        $headerImageMax     = config('zevolifesettings.fileSizeValidations.webinar.header_image', 2048);
        $headerImageRules   = array();
        $headerImage = function () {
            $thisId             = $this->route('webinar')->id;
            $getWebinarData     = Webinar::find($thisId);
            if (!empty($getWebinarData)){
                return $getWebinarData->header_image_name;
            } 
        };
        $headerImageRules = [
        ...
        $rules['webinar_category'] = 'required|integer';
        ...
        $rules['webinar_file']      = "sometimes|nullable|mimetypes:video/mp4|max:{$trackMax}";
        $rules['webinar_youtube']   = "required_if:webinar_type,2|sometimes|nullable|url|youtube_url";
